...
agents rely on:
  • Auto-waiting: clicks/fills wait for the element to be visible, enabled and stable
    before acting — no flaky time.sleep() guesses.
  • Fast, reliable navigation with explicit load states.
  • First-class locators and role/text queries.

Why a dedicated worker thread?
  Playwright's *sync* API objects are bound to the thread that created them, and the
  browser must survive across several HTTP requests (navigate → the user logs in →
...
_NEXT_WORDS = ["Next", "Sign in", "Log in", "Login", "Continue",
               "التالي", "تسجيل الدخول", "دخول", "متابعة"]
# Words that label a "submit / search / inquire" button (EN + AR) — used so the agent can
...
_SUBMIT_WORDS = ["Search", "Submit", "Inquiry", "Inquire", "Enquire", "Verify", "Send Request",
                 "بحث", "ابحث", "إرسال", "ارسال", "تحقق", "استعلام", "عرض"]
...
# CORE stealth — SAFE on ANY browser we launch (bundled Chromium AND real Chrome via channel=
...
# (NOT applied when attached to the user's own Chrome over CDP — that browser isn't automated.)
_STEALTH_CORE_JS = r"""
(() => {
  const def = (obj, prop, get) => { try { Object.defineProperty(obj, prop, {get, configurable: true}); } catch (e) {} };

  // navigator.webdriver — the single biggest automation tell.
  ...
  // An iframe's contentWindow can leak the automation flag — propagate the override.
  try {
...
# FINGERPRINT stealth — ONLY for BUNDLED Chromium, where the real values ARE tells (empty
...
(() => {
  ...
  // WebGL vendor/renderer — headless Chromium reports "Google SwiftShader/Mesa"; spoof a real GPU.
  try {
    const spoof = (proto) => {
...
# Error indicators on a page (EN + AR) — used to tell whether an action actually worked.
_ERROR_HINTS = ("error", "invalid", "incorrect", "not found", "try again", "failed",
                "wrong", "denied", "rejected", "خطأ", "غير صحيح", "فشل", "حاول مرة")
...
class BrowserSession:
    ...
    def _worker(self) -> None:
        try:
            ...
            # "Chromium" — a tell). For REAL Chrome (channel='chrome') we let it send its OWN UA so
            ...
            # into Google) a high reCAPTCHA reputation — so the score-based reCAPTCHA Enterprise on
            ...
            # --remote-debugging-port=9222 --user-data-dir="%LOCALAPPDATA%\fanar-chrome"), sign into
            ...
            if not cdp_url and os.getenv("BROWSER_CDP_AUTODETECT", "true").lower() != "false":
                if _port_open("127.0.0.1", 9222):     # a debuggable Chrome is already running → use it
            ...
            if cdp_url:
                ...
                except Exception:  # noqa: BLE001 — Chrome not started with the debug port → fall back
                    self._browser = None
                    self._ctx = None
            ...
            if persistent:
                # Headful and headless must NOT share a profile dir — mixing them corrupts the
                ...
                for udd in (main_dir, str(WORKSPACE / f"chrome_profile_{self.session_id}")):
                    for channel in ("chrome", None):
                        ...
                        try:
                            ...
                            break
                        except Exception:  # noqa: BLE001 — locked profile / no Chrome channel → try next
                            self._ctx = None
            ...
            # bundled Chromium (on real Chrome they'd create a mismatch tell — see the script docs).
            if not self.cdp:
                try:
            ...
            # launched persistent context starts with one blank page — reuse it (no 2nd window).
            if self.cdp:
        ...
        while True:
            ...
            if op is _STOP:
                ...
                # context — just disconnect (pw.stop()). Otherwise close what we launched.
                if not self.cdp:
                    try:
                        self._ctx.close()
                    except Exception:  # noqa: BLE001
    ...
    def _settle(self) -> None:
        ...
        # quiet down. We do NOT block on every image finishing — images keep streaming
    ...
    def _clear_marks(self) -> None:
        try:
            self._page.evaluate("() => document.querySelectorAll('.__fanar_mark').forEach(e => e.remove())")
        except Exception:  # noqa: BLE001
            pass

    def _human_mouse_to(self, x: float, y: float) -> None:
        """Move the mouse toward (x, y) in several small steps with a tiny pause — real pointer
    ...
    def _human_type(self, selector: str, value: str) -> bool:
        ...
        value swap — the latter reads as a bot). Falls back to the native setter for SPA inputs
        ...
        try:
            ...
            except Exception:  # noqa: BLE001 — older Playwright spelling
    ...
    def fill_date(self, n: int, text: str, fmt: str = "yyyy/mm/dd") -> dict[str, Any]:
        """Fill a date field robustly — handles plain text inputs, native
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            except Exception:  # noqa: BLE001 — marks may be stale; re-label and retry
                s._page.evaluate(_ANNOTATE_JS)
                loc = s._page.locator(sel).first
                handle = loc.element_handle(timeout=5000)
    ...
    def fill_date_smart(self, value: str, synonyms: list[str] | None = None) -> dict[str, Any]:
        """Fill a date by VALUE (no box number) — robust to ANY date-input shape:
        ...
        `synonyms` (e.g. ['date of birth','الميلاد']) help locate the single field."""
        def op(s: "BrowserSession") -> dict[str, Any]:
            p = _date_parts(value)
            if not p:
    ...
    def fill_login(self, username: str = "", password: str = "", submit: bool = True,
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if has_password:
                if submit and not needs_captcha:
                    ...
                    if res.get("submit_found"):
                        try:
                            loc = s._page.locator('[data-fanar-login-submit]').first
                            if humanize:
                                box = _safe(lambda: loc.bounding_box(), None)
                                if box:
                            ...
                            clicked = bool(_safe(lambda: s._page.evaluate(
                                "() => { const b = document.querySelector('[data-fanar-login-submit]');"
            ...
            # Still on a login page (or an error showed) → the login probably failed.
    ...
    def submit_inquiry_form(self) -> dict[str, Any]:
        ...
        inquiry form — preferring the one with the captcha box). Never the site-wide searchQuery
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if info.get("found") and info.get("button"):
                try:
                    s._page.locator('[data-fanar-submit]').first.click(timeout=5000)
                    clicked = True
                except Exception:  # noqa: BLE001 — fall back to a direct JS click on the tagged button
                    clicked = bool(_safe(lambda: s._page.evaluate(
                        "() => { const b = document.querySelector('[data-fanar-submit]');"
                        " if (b) { b.click(); return true; } return false; }"), False))
            s._settle()
            st = s._annotated_state()
            after = (st.get("url"), st.get("title"), len(_safe(lambda: s._page.content(), "")))
    ...
    def detect_otp(self, field_selector: str = "#otp-field", form_selector: str = "#mfaOtpFrm",
        ...
        """Wait briefly for a one-time-code (OTP) page to appear — the given field/form, or a
    ...
    def fill_otp(self, code: str, field_selector: str = "#otp-field",
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if sub.get("found"):
                try:
                    s._page.locator('[data-fanar-otp-submit]').first.click(timeout=5000)
                    clicked = True
                except Exception:  # noqa: BLE001
                    clicked = bool(_safe(lambda: s._page.evaluate(
                        "() => { const b = document.querySelector('[data-fanar-otp-submit]');"
                        " if (b) { b.click(); return true; } return false; }"), False))
            if not clicked:
    ...
    def fill_service_dialog(self, email: str = "", address_type: str = "Home Address",
        ...
        ("English"), fill the email field with the user's email (injected by code — the model never
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if click_pay and res.get("pay_found"):
                try:
                    s._page.locator('[data-fanar-pay-btn]').first.click(timeout=5000)
                    pay_clicked = True
                except Exception:  # noqa: BLE001
                    pay_clicked = bool(_safe(lambda: s._page.evaluate(
                        "() => { const b = document.querySelector('[data-fanar-pay-btn]');"
                        " if (b) { b.click(); return true; } return false; }"), False))
            s._settle()
            st = s._annotated_state()
    ...
    def expand_all(self, labels: list[str] | None = None) -> dict[str, Any]:
        ...
        reveal hidden detail — used before reading the PHCC medications list (Dose/Frequency/Route
    ...
    def confirm_payment_method(self, radio_id: str = "", card_label: str = "") -> dict[str, Any]:
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if info.get("pay_found"):
                try:
                    s._page.locator('[data-fanar-pay-btn]').first.click(timeout=4000)
                    clicked = True
                except Exception:  # noqa: BLE001
                    clicked = bool(_safe(lambda: s._page.evaluate(
                        "() => { const b = document.querySelector('[data-fanar-pay-btn]');"
    ...
    def fill_id_card_form(self, service_type: str = "") -> dict[str, Any]:
        """Replace Lost/Damaged ID Card — the "Expatriate Data" page: tick the Service Type radio
        ...
        directly on the DOM by their labels — deterministic so the model can't skip them."""
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if info.get("next_found"):
                s._page.wait_for_timeout(200)
                try:
                    s._page.locator('[data-fanar-next-btn]').first.click(timeout=4000)
                    next_clicked = True
                except Exception:  # noqa: BLE001
                    next_clicked = bool(_safe(lambda: s._page.evaluate(
                        "() => { const b = document.querySelector('[data-fanar-next-btn]');"
                        " if (b) { b.click(); return true; } return false; }"), False))
            s._settle()
            ...
            for _ in range(8):
                ...
                if m.get("found"):
                    try:
                        s._page.locator('[data-fanar-modal-btn]').first.click(timeout=4000)
                        ok_clicked = True
                    except Exception:  # noqa: BLE001
                        ok_clicked = bool(_safe(lambda: s._page.evaluate(
                            "() => { const b = document.querySelector('[data-fanar-modal-btn]');"
                            " if (b) { b.click(); return true; } return false; }"), False))
                    if ok_clicked:
                        break
    ...
    def read_editable_form(self) -> dict[str, Any]:
        ...
        options}]}. Scans the topmost modal if one is open, else the whole document — and RETRIES a
    ...
    def click_modal(self, labels: list[str] | None = None, timeout_ms: int = 6000,
        ...
        modal. Never clicks Close/Cancel/× or a language toggle."""
        labels = [str(x) for x in (labels or []) if x]
    ...
    def click_smart(self, labels: list[str] | None = None) -> dict[str, Any]:
        """Robustly click a navbar/header control (e.g. the 'English' language link) — scroll to
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if info.get("found"):
                try:
                    s._page.locator('[data-fanar-smartclick]').first.click(timeout=4000)
                    clicked = True
                except Exception:  # noqa: BLE001
                    clicked = bool(_safe(lambda: s._page.evaluate(
                        "() => { const b = document.querySelector('[data-fanar-smartclick]');"
                        " if (b) { b.click(); return true; } return false; }"), False))
            s._settle()
    ...
    def click_tab(self, labels: list[str] | None = None) -> dict[str, Any]:
        ...
        ([role=tab], .nav-tabs, [data-toggle=tab], …), matches a label (exact word or substring),
        scrolls it into view and clicks it — then returns the freshly-labelled page (clicking a tab
        ...
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            if info.get("found"):
                try:
                    s._page.locator('[data-fanar-tab]').first.click(timeout=4000)
                    clicked = True
                except Exception:  # noqa: BLE001
                    clicked = bool(_safe(lambda: s._page.evaluate(
                        "() => { const b = document.querySelector('[data-fanar-tab]');"
                        " if (b) { b.click(); return true; } return false; }"), False))
            s._settle()
    ...
    def _click_submit(self) -> bool:
        ...
        page = self._page
        # 1) type=submit — the most reliable, language-independent signal.
    ...
    def capture_captcha(self) -> dict[str, Any]:
        ...
        button — so the app can show the image, let the user type the code, and we fill + submit
        it ourselves. Degrades to {found: False} when no captcha image is recognisable."""
        def op(s: "BrowserSession") -> dict[str, Any]:
            s._page.evaluate(_ANNOTATE_JS)  # ensure inputs/buttons carry data-fanar-mark
            info = _safe(lambda: s._page.evaluate(_CAPTCHA_JS), {}) or {}
            image = ""
            if info.get("found"):
                try:
                    png = s._page.locator('[data-fanar-captcha-img]').first.screenshot(timeout=4000)
                    image = "data:image/png;base64," + base64.b64encode(png).decode()
                except Exception:  # noqa: BLE001 — fall back to a clip of the reported rect
    ...
    def click(self, target: str) -> dict[str, Any]:
        def op(s: "BrowserSession") -> dict[str, Any]:
            ...
            getters = [
                ...
                # logo by alt text or title and click it — the click bubbles to the selectable box.
            ...
            # text/role lookups miss it and `locator(target)` treats it as a tag — so also try it as
    ...
    def live_screenshot(self) -> bytes:
        ...
        poll during a navigation/redirect would otherwise time out and return nothing → the UI fell
        ...
        def op(s: "BrowserSession") -> bytes:
            try:
                ...
                return png
            except Exception:  # noqa: BLE001 — mid-navigation / page closed → reuse the last frame
...
def _date_parts(text: str) -> tuple[str, str, str] | None:
    ...
    if len(a) == 4:                     # yyyy m d
        y, m, d = a, b, c
    elif len(c) == 4:                   # d m yyyy  (or m d yyyy — assume d/m, common in Qatar)
        d, m, y = a, b, c
    else:
        return None
    try:
...
# tag it with data-fanar-mark so we can click it by number.
# --------------------------------------------------------------------------- #
_ANNOTATE_JS = r"""
() => {
  document.querySelectorAll('.__fanar_mark').forEach(e => e.remove());
  ...
  // stuck clicking covered fields — exactly the MOI "serviceModal" certificate-form failure.
  ...
  for (const el of els) {
    ...
    i++;
    el.setAttribute('data-fanar-mark', i);
...
(args) => {
  ...
  const AR_MONTHS = ['يناير','فبراير','مارس','أبريل','مايو','يونيو','يوليو','أغسطس',
  ...
  const classify = (el) => {
    ...
    if (/(^|[^a-z])(year|yyyy|سنة|عام)([^a-z]|$)/.test(t)) return 'year';
    if (/(^|[^a-z])(month|mm|شهر)([^a-z]|$)/.test(t)) return 'month';
    if (/(^|[^a-z])(day|dd|يوم)([^a-z]|$)/.test(t)) return 'day';
  ...
  // 2b) Heuristic split: three small numeric boxes (maxlength 1–4). If a synonym group of
  ...
  for (const el of inputs) {
    ...
    if (/(date|تاريخ|dd[\/\-.]mm|yyyy)/.test(t.toLowerCase())) {
...
(args) => {
  ...
  out.needs_captcha = Array.from(root.querySelectorAll('img')).some(im =>
    /captcha|verif|securimage|رمز|التحقق/i.test((im.src || '') + ' ' + (im.alt || '') + ' ' + (im.id || '') + ' ' + (im.className || '')));
  ...
  const searchy = (el) => /search|captcha|code|otp|بحث|رمز/.test(attrs(el));
  ...
  const score = (el) => { const a = attrs(el); let s = 0;
    ...
    if (/qid|national|civil|(^|[^a-z])id([^a-z]|$)|الهوية|المستخدم|البريد/.test(a)) s += 3;
  ...
  try {
    document.querySelectorAll('[data-fanar-login-user]').forEach(e => e.removeAttribute('data-fanar-login-user'));
    document.querySelectorAll('[data-fanar-login-pass]').forEach(e => e.removeAttribute('data-fanar-login-pass'));
  } catch (e) {}
  if (userEl) {
    try { userEl.setAttribute('data-fanar-login-user', '1'); } catch (e) {}
    out.user_found = true;
  ...
  if (pwds.length) {
    try { pwds[0].setAttribute('data-fanar-login-pass', '1'); } catch (e) {}
    if (P && !args.tag_only) { setVal(pwds[0], P); out.filled.push('password'); out.pwd_len = (pwds[0].value || '').length; }
  }

  // Locate the form's SUBMIT/Continue button and tag it so Playwright can click exactly it —
  // never a language toggle (English/العربية), which is the bug we are fixing. Search the login
  // container first, then the document.
  try {
    document.querySelectorAll('[data-fanar-login-submit]').forEach(e => e.removeAttribute('data-fanar-login-submit'));
    const SUB_RE = /continue|next|log ?in|sign ?in|submit|proceed|متابعة|التالي|تسجيل الدخول|دخول|إرسال|تأكيد|دخـول/i;
    const LANG_RE = /english|arabic|العربية|عربي|language|اللغة|\ben\b|\bar\b/i;
    ...
    if (sb) { sb.setAttribute('data-fanar-login-submit', '1'); out.submit_found = true; out.submit_label = btnLabel(sb).slice(0, 30); }
  } catch (e) {}
  return out;
}
"""
...
(args) => {
  ...
  document.querySelectorAll('[data-fanar-smartclick]').forEach(e => e.removeAttribute('data-fanar-smartclick'));
  ...
  el.setAttribute('data-fanar-smartclick', '1');
...
(args) => {
  ...
  const txt = (el) => ((el.innerText || el.textContent || '') + ' ' + (el.getAttribute('aria-label') || '') +
    ' ' + (el.title || '') + ' ' + (el.value || '')).replace(/\s+/g, ' ').trim().toLowerCase();
  document.querySelectorAll('[data-fanar-tab]').forEach(e => e.removeAttribute('data-fanar-tab'));
  ...
  if (!el.matches(CLICKABLE)) {
    ...
    // clickable control — a descendant first (the <a> inside an <li>), else a clickable ancestor.
  ...
  el.setAttribute('data-fanar-tab', '1');
  try { el.scrollIntoView({block: 'center'}); } catch (e) {}
  return {found: true, text: txt(el).slice(0, 40)};
}
"""
...
(args) => {
  ...
  el = find(['card number', 'cardnumber', 'card_no', 'cardno', 'ccnumber', 'cc-number', 'pan', 'رقم البطاقة']);
  if (el && card.card_number) { used.add(el); setVal(el, String(card.card_number)); out.filled.push('card_number'); }
  el = find(['cardholder', 'card holder', 'name on card', 'holdername', 'cc-name', 'ccname', 'اسم حامل']);
  if (el && card.cardholder_name) { used.add(el); setVal(el, String(card.cardholder_name)); out.filled.push('cardholder_name'); }

  // Expiry — a single MM/YY field, else split Month + Year that may be <select> dropdowns (QPAY).
  if (card.expiry) {
    ...
    if (combined && combined.tagName !== 'SELECT') { used.add(combined); setVal(combined, String(card.expiry)); out.filled.push('expiry'); }
      ...
      const monthEl = find(['expiry month', 'expmonth', 'exp_month', 'exp-month', 'expirymonth', 'cardmonth', 'month', 'شهر']);
      ...
      const yearEl = find(['expiry year', 'expyear', 'exp_year', 'exp-year', 'expiryyear', 'cardyear', 'year', 'سنة']);
  ...
  el = find(['cvv', 'cvc', 'cvv2', 'cvc2', 'security code', 'securitycode', 'cc-csc', 'رمز الأمان', 'رمز التحقق']);
  if (el && card.cvv) { used.add(el); setVal(el, String(card.cvv)); out.filled.push('cvv'); }
  el = find(['zip', 'postal', 'postcode', 'post code', 'الرمز البريدي']);
...
# element screenshot), the code INPUT box, and the SUBMIT button — returning the
...
() => {
  const vis = (el) => { const r = el.getBoundingClientRect(); const st = getComputedStyle(el);
    return r.width > 4 && r.height > 4 && st.visibility !== 'hidden' && st.display !== 'none'; };
  const re = /captcha|verif|securimage|رمز|التحقق/i;
  ...
  document.querySelectorAll('[data-fanar-captcha-img]').forEach(e => e.removeAttribute('data-fanar-captcha-img'));
  const out = {found: false};
  if (!cimg) return out;
  cimg.setAttribute('data-fanar-captcha-img', '1');
  out.found = true;
  ...
  if (best) { const mk = parseInt(best.getAttribute('data-fanar-mark') || ''); if (!isNaN(mk)) out.input_mark = mk; }

  // submit/search button
  const btns = Array.from(document.querySelectorAll('button, input[type=submit], input[type=image], a[role=button], a')).filter(vis);
  const sre = /submit|search|verify|inquir|enquir|بحث|ابحث|إرسال|ارسال|تحقق|استعلام/i;
  ...
  if (sb) { const mk = parseInt(sb.getAttribute('data-fanar-mark') || ''); if (!isNaN(mk)) out.submit_mark = mk; }
  return out;
}
"""
...
# (a.k.a. search) button INSIDE THAT FORM ONLY — never the site-wide searchQuery
# form or any button outside the input form.
# --------------------------------------------------------------------------- #
_SUBMIT_IN_FORM_JS = r"""
() => {
  const SUBMIT_RE = /submit|search|inquir|enquir|بحث|ابحث|إرسال|ارسال|تحقق|استعلام|عرض/i;
  const RESET_RE  = /reset|clear|cancel|back|إلغاء|مسح|الغاء|رجوع/i;
  ...
  const hasCaptcha = (f) => Array.from(f.querySelectorAll('input, img')).some(el =>
    /captcha|verif|رمز|التحقق|code/i.test((el.name || '') + ' ' + (el.id || '') + ' ' +
  ...
  // fees page has separate frmPlateNum / frmQid / frmCpy forms — only the active tab's form is
  ...
  document.querySelectorAll('[data-fanar-submit]').forEach(e => e.removeAttribute('data-fanar-submit'));
  ...
  btn.setAttribute('data-fanar-submit', '1');
...
# --------------------------------------------------------------------------- #
# OTP locator (runs in the page): find the one-time-code input — the explicit field
...
(args) => {
  ...
  const tag = (el) => { if (el.id) return '#' + el.id; el.setAttribute('data-fanar-otp-input', '1'); return '[data-fanar-otp-input]'; };
  ...
  const re = /otp|one.?time|verification|verify.?code|auth.?code|mfa|2fa|رمز|التحقق/i;
...
(args) => {
  ...
  const vis = (el) => { const r = el.getBoundingClientRect(); const st = getComputedStyle(el);
    return r.width > 2 && r.height > 2 && st.visibility !== 'hidden' && st.display !== 'none'; };
  const SUB  = /continue|next|submit|verify|confirm|log ?in|sign ?in|proceed|متابعة|التالي|تأكيد|إرسال|دخول|تحقق/i;
  const LANG = /english|arabic|العربية|عربي|language|اللغة|\ben\b|\bar\b/i;
  const SKIP = /reset|clear|cancel|back|resend|re-send|إلغاء|مسح|رجوع|إعادة|إعادة إرسال/i;
  const label = (b) => ((b.innerText || '') + ' ' + (b.value || '') + ' ' + (b.getAttribute('aria-label') || '') +
    ' ' + (b.id || '') + ' ' + (b.className || '') + ' ' + (b.title || '')).trim();
  document.querySelectorAll('[data-fanar-otp-submit]').forEach(e => e.removeAttribute('data-fanar-otp-submit'));
  ...
  b.setAttribute('data-fanar-otp-submit', '1');
...
# and click its primary button (e.g. "Login"/"Continue") — footer first — never Close/Cancel/×,
...
(args) => {
  ...
  if (select) {
    ...
    // Prefer an EXACT label, then a TIGHT match (label not much longer than the target — avoids
    ...
    if (o) {
      ...
      if (!o.checked) {   // framework-controlled inputs (React/Vue) can ignore .click() — force it
        try { o.checked = true;
  ...
  document.querySelectorAll('[data-fanar-modal-btn]').forEach(e => e.removeAttribute('data-fanar-modal-btn'));
  const SKIP = /close|cancel|dismiss|back|إغلاق|إلغاء|رجوع|×|✕|✖/i;
  const LANG = /english|arabic|العربية|عربي|language|اللغة|\ben\b|\bar\b/i;
  ...
  if (!b) return out;
  b.setAttribute('data-fanar-modal-btn', '1');
...
# select the language ("English"), and fill the email field — all INSIDE the topmost modal so we
...
(args) => {
  ...
  // 2) Language "English" — a radio, else a <select> option.
  ...
  if (args.click_pay) {
    document.querySelectorAll('[data-fanar-pay-btn]').forEach(e => e.removeAttribute('data-fanar-pay-btn'));
    ...
    if (pb) { pb.setAttribute('data-fanar-pay-btn', '1'); out.pay_found = true; out.pay_label = (textOf(pb) || attrOf(pb)).trim().slice(0, 40); }
  }
  return out;
}
"""
...
() => {
  ...
  const FEE_RE = /total\s*fee|fee\s*amount|total\s*amount|amount\s*(to\s*pay|due|payable)|service\s*fee|grand\s*total|الرسوم|المبلغ|الإجمالي/i;
  ...
  const push = (s) => {
    ...
    if (/^[\s:.\-–—|*/\\]+$/.test(v)) return;            // pure punctuation
  ...
  const walk = (node) => {
    ...
    if (node.children.length === 0) { push(node.textContent); return; }   // leaf element → its text
  ...
  // ---- 3) DETERMINISTIC input-based pairing (PRIMARY) — the MOI review page is a Bootstrap
  ...
  //      unit label. Pair every value-holding input/select with its OWN label → exact rows. (This
  //      is what the ordered-line/positional heuristic could never get right.)
  const UNIT = /^(q\.?\s*r\.?|qar|qr|﷼|ر\.?\s*ق|riyals?|[:*]+)$/i;   // currency/punctuation labels, not a field name
  const labelOf = (el) => {
  ...
  // ---- 4) Total fee — prefer the labelled input; else a MONEY value near a fee label in `lines`.
  const MONEY_LINE = /^\s*(\d[\d,\s.]*)\s*(q\.?\s*r\.?|qar|qr|﷼|ر\.?\s*ق|riyals?)?\s*$/i;
  if (!total) {
    for (let i = 0; i < lines.length; i++) {
      ...
      const mi = after.match(/\d[\d,\s.]*\s*(q\.?\s*r\.?|qar|qr|﷼|riyals?)?/i);
  ...
  if (total && /^\s*[\d,.\s]+\s*$/.test(total)) total = clean(total) + ' QR';   // bare number → add currency
  ...
  // NOTE: no positional label/value fallback here on purpose — on a NOT-YET-POPULATED page it
  // pairs adjacent labels as garbage (Name→"RP Expiry Date"). When there are no labelled inputs
...
(args) => {
  ...
  let clicked = 0;
  for (const el of els) {
    if (el.hasAttribute('data-fanar-expanded')) continue;     // don't re-click (would collapse it)
    const t = txt(el);
    if (!t || t.length > 45) continue;
    if (!labels.some(l => t.includes(l))) continue;
    el.setAttribute('data-fanar-expanded', '1');
...
(args) => {
  ...
  // Find the "Pay" button — prefer one inside the same dialog as the radio; never Close/Cancel.
  let scope = document;
  ...
  let pay = btns.find(b => /^pay$/i.test(txt(b))) || btns.find(b => /\bpay\b/i.test(txt(b)))
         || btns.find(b => (b.getAttribute('type') || '').toLowerCase() === 'submit');
  if (pay) pay.setAttribute('data-fanar-pay-btn', '1');
...
(args) => {
  ...
  // Service Type — prefer the KNOWN MOI ids, else match by label ("lost"/"damaged").
  let svc = null;
  ...
  // QID Options — "My QID" by KNOWN id, else by label (never the family-member/expatriate one).
  ...
  document.querySelectorAll('[data-fanar-next-btn]').forEach(e => e.removeAttribute('data-fanar-next-btn'));
  ...
  if (nx) { nx.setAttribute('data-fanar-next-btn', '1'); out.next_found = true; }
  return out;
}
"""
...
() => {
  ...
  // container used to miss the address form — different portals nest it differently).
  const modalSel = '.modal.show, .modal.in, .modal[style*="display: block"], [role=dialog], .ui-dialog, [class*=modal][class*=show], [class*=modal][class*=open]';
  const modals = Array.from(document.querySelectorAll(modalSel)).filter(vis);
  ...
  document.querySelectorAll('[data-fanar-edit]').forEach(e => e.removeAttribute('data-fanar-edit'));
  ...
  Array.from(root.querySelectorAll('input, select, textarea')).forEach((el) => {
    ...
    seen[nm] = 1;
    // TAG the exact element we surface, so we can fill THIS element back by tag — robust against
    // name/id mismatches, duplicate names, and case differences.
    el.setAttribute('data-fanar-edit', String(i));
...
_FILL_EDITABLE_JS = r"""
(args) => {
  const vals = args.values || {};   // keyed by the data-fanar-edit index from _READ_EDITABLE_JS
  ...
  Object.keys(vals).forEach((k) => {
    const el = document.querySelector('[data-fanar-edit="' + (window.CSS && CSS.escape ? CSS.escape(k) : k) + '"]');
