...
import { makeT, fieldLabel, type Lang } from "./i18n";
import type { PaymentField, ProfileField, SavedCredential } from "./types";

type Section = "info" | "payment" | "credentials";

// "Saved Information" hub — one drawer with three sections the user switches between:
//   • My Info     — optional identity fields the agent reuses to auto-fill forms (+ QID upload)
//   • Payment     — an optional saved card for checkout/fee-payment tasks
//   • Credentials — saved logins (Chrome-style), mapped to a site, used to sign in automatically
// Everything is OPTIONAL and stored locally only; nothing here is sent to the AI model.
export function ProfilePanel({
  ...
  const labelFor = (key: string) => fieldLabel(lang, key, fields.find((f) => f.key === key)?.label || key);
              ...
              <div className="mb-5 rounded-xl border border-dashed border-line bg-bg/40 p-4">
                ...
                {detected && (
                  <p className={`mt-2 text-[11px] ${detected.length ? "text-accent" : "text-muted-foreground"}`}>
                    {detected.length
                      ? `${t("panel.detectedFilled")} ${detected.map(labelFor).join(", ")}`
                      : t("panel.noFieldsRead")}
                  </p>
                )}
              </div>
          ...
          {section === "payment" && (
            <>
              ...
              <div className="space-y-4">
                {paymentFields.map((f) => (
                  <div key={f.key}>
                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">{fieldLabel(lang, f.key, f.label)}</label>
                    <input
...
// ── My Credentials: saved logins mapped to a site (view / edit / delete / add) ──
            ...
            <div key={c.host} className="rounded-xl border border-line bg-bg/40 p-3">
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                <button
                  ...
                  title={t("panel.show") + " / " + t("panel.hide")}
                >
                  {reveal[c.host] ? t("panel.hide") : t("panel.show")}
                </button>
              </div>
      ...
      <div className="mt-5 rounded-xl border border-dashed border-line bg-bg/40 p-3">
        ...
        {(["url", "username", "password", "label"] as const).map((k) => (
          <input
            ...
            placeholder={k === "url" ? "site url (e.g. https://eservices.moi.gov.qa)" : k === "label" ? t("panel.labelOptional") : t(`panel.${k}`)}
            autoComplete="off"
            dir="auto"
            value={adding[k]}
